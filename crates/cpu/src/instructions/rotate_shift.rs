...
impl OpConfigBuilder {
    ...
    pub const fn set_z(mut self) -> Self {
        self.config.set_z = true;
        self
    }

    pub const fn add_carry(mut self) -> Self {
        self.config.add_carry = true;
        self
    }

    pub const fn repeat_tail(mut self) -> Self {
        self.config.repeat_tail = true;
        self
    }

    pub const fn build(self) -> OpConfig {
        self.config
    }
}
...
impl Op {
    ...
    pub fn execute(&self, flags: u8, config: &OpConfig) -> (u8, u8) {
        let mut result = match self {
            Self::RotateLeft(x) => x.rotate_left(1),
            Self::RotateRight(x) => x.rotate_right(1),
            Self::ShiftLeft(x) => x << 1,
            Self::ShiftRight(x) => x >> 1,
        };
        let (to_carry, from_carry, tail_bit) = match self {
            Self::RotateLeft(x) | Self::ShiftLeft(x) => (x & 0x80, 0x01, x & 0x01),
            Self::RotateRight(x) | Self::ShiftRight(x) => (x & 0x01, 0x80, x & 0x80),
        };
...
mod tests {
    ...
    #[test]
    fn rlca_rotates_the_a_register_left() {
        let (mut registers, mut memory, mut cpu_flags) = setup();
    ...
    #[test]
    fn rla_rotates_the_a_register_left() {
        let (mut registers, mut memory, mut cpu_flags) = setup();
    ...
    #[test]
    fn rrca_rotates_the_a_register_right() {
        let (mut registers, mut memory, mut cpu_flags) = setup();
    ...
    #[test]
    fn rra_rotates_the_a_register_left() {
        let (mut registers, mut memory, mut cpu_flags) = setup();
    ...
    #[test]
    fn rlc_returns_the_correct_machine_cycles() {
        let (mut registers, mut memory, mut cpu_flags) = setup();

        for operand in 0..8 {
            let cycles = RotateShift::RLC(operand)
                .execute(&mut registers, &mut memory, &mut cpu_flags)
                .unwrap();
    ...
    #[test]
    fn rlc_rotates_the_correct_register() {
        ...
        for operand in 0..8 {
            ...
            if operand == 0b110 {
                assert_eq!(
                    ...
                    "Incorrect result for (HL)"
                );
            } else {
                let r: SingleRegister = operand.try_into().unwrap();
                assert_eq!(
                    expected,
                    registers.get_single(&r),
                    "Incorrect result for register {r:?}"
                );
            }
    ...
    #[test]
    fn rlc_handles_flags_correctly() {
        let (mut registers, mut memory, mut cpu_flags) = setup();
    ...
    #[test]
    fn rrc_returns_the_correct_machine_cycles() {
        let (mut registers, mut memory, mut cpu_flags) = setup();

        for operand in 0..8 {
            let cycles = RotateShift::RRC(operand)
                .execute(&mut registers, &mut memory, &mut cpu_flags)
                .unwrap();
    ...
    #[test]
    fn rrc_rotates_the_correct_register() {
        use std::convert::TryInto;
    ...
    #[test]
    fn rrc_handles_flags_correctly() {
        let (mut registers, mut memory, mut cpu_flags) = setup();
    ...
    #[test]
    fn rl_returns_the_correct_machine_cycles() {
        let (mut registers, mut memory, mut cpu_flags) = setup();

        for operand in 0..8 {
            let cycles = RotateShift::RL(operand)
                .execute(&mut registers, &mut memory, &mut cpu_flags)
                .unwrap();
    ...
    #[test]
    fn rl_rotates_the_correct_register_to_the_left() {
        use std::convert::TryInto;
    ...
    #[test]
    fn rr_returns_the_correct_machine_cycles() {
        let (mut registers, mut memory, mut cpu_flags) = setup();

        for operand in 0..8 {
            let cycles = RotateShift::RR(operand)
                .execute(&mut registers, &mut memory, &mut cpu_flags)
                .unwrap();
    ...
    #[test]
    fn rr_rotates_the_correct_register_to_the_right() {
        use std::convert::TryInto;
    ...
    #[test]
    fn sla_returns_the_correct_machine_cycles() {
        let (mut registers, mut memory, mut cpu_flags) = setup();

        for operand in 0..8 {
            let cycles = RotateShift::SLA(operand)
                .execute(&mut registers, &mut memory, &mut cpu_flags)
                .unwrap();
    ...
    #[test]
    fn sla_shifts_the_correct_register_to_the_left() {
        use std::convert::TryInto;
    ...
    #[test]
    fn sla_handles_flags_correctly() {
        let (mut registers, mut memory, mut cpu_flags) = setup();
    ...
    #[test]
    fn sra_returns_the_correct_machine_cycles() {
        let (mut registers, mut memory, mut cpu_flags) = setup();

        for operand in 0..8 {
            let cycles = RotateShift::SRA(operand)
                .execute(&mut registers, &mut memory, &mut cpu_flags)
                .unwrap();
    ...
    #[test]
    fn sra_shifts_the_correct_register_to_the_right() {
        use std::convert::TryInto;
    ...
    #[test]
    fn sra_repeats_the_seventh_bit() {
        use std::convert::TryInto;
    ...
    #[test]
    fn sra_handles_flags_correctly() {
        let (mut registers, mut memory, mut cpu_flags) = setup();
    ...
    #[test]
    fn srl_returns_the_correct_machine_cycles() {
        let (mut registers, mut memory, mut cpu_flags) = setup();

        for operand in 0..8 {
            let cycles = RotateShift::SRL(operand)
                .execute(&mut registers, &mut memory, &mut cpu_flags)
                .unwrap();
    ...
    #[test]
    fn srl_shifts_the_correct_register_to_the_right() {
        use std::convert::TryInto;
    ...
    #[test]
    fn srl_does_not_repeat_the_seventh_bit() {
        use std::convert::TryInto;
    ...
    #[test]
    fn srl_handles_flags_correctly() {
        let (mut registers, mut memory, mut cpu_flags) = setup();
    ...
    #[test]
    fn swap_returns_the_correct_machine_cycles() {
        let (mut registers, mut memory, mut cpu_flags) = setup();

        for operand in 0..8 {
            let cycles = RotateShift::SWAP(operand)
                .execute(&mut registers, &mut memory, &mut cpu_flags)
                .unwrap();
    ...
    #[test]
    fn swap_shifts_the_correct_register_to_the_right() {
        use std::convert::TryInto;
    ...
    #[test]
    fn swap_handles_flags_correctly() {
        let (mut registers, mut memory, mut cpu_flags) = setup();
